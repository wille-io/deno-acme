...
export function createHttpRequestQueue()
{
  return new HttpRequestQueue();
}
  ...
  : Promise<{domainCertificates: DomainCertificate[], pemAccountKeys: AccountKeys}>
{
  ...
  return {
    domainCertificates: await new ACMEHttp(session, domains, options?.yourEmail, options?.csrInfo, options?.httpRequestQueue).getCertificates(),
    ...
    do
    {
      ...
      if (options?.waitForAcmeStatus)
      {
        ...
        if (["pending", "processing"].includes(status)) // TODO: special 'pending' handling?
        {
          ...
          console.log(`acme request status is '${status}' - trying again in ${retryAfter} seconds`);
          waitTime += retryAfter;
    ...
    : Promise<DomainCertificate[]>
  {
    ...
    for (const domain of this.domains)
    {
      try
      {
        ...
        // check order
        //const x =
        await this.session.post(orderUrl);
...
class DefaultHttpListener
{
    ...
    this.server = Deno.serve(
      {
        ...
        onListen: () =>
        {
          // console.debug("onListen");
          this.ready = true;
          if (this.resolveOnListen)
          {
            this.resolveOnListen();
          }
        }
      },
      async (request) =>
  ...
  static async create()
  {
    const instance = new DefaultHttpListener();

    try
    {
      const onListenPromise: Promise<void> = new Promise((onListenResolver) =>
      {
        // console.debug("new onListenPromise");
        instance.resolveOnListen = onListenResolver;
      });
      ...
      await onListenPromise;
      return instance;
    }
    catch(_e)
    {
      console.error("creating DefaultHttpListener failed", _e);
      instance.stop();
      throw _e;
    }
  }

  setHttpRequestQueue(httpRequestQueue: HttpRequestQueue)
  {
...
class ACMEHttp extends ACMEBase
{
  ...
  async newChallenges(_domain: Domain, auths: Auth[])
  {
    ...
    let defaultHttpListener: DefaultHttpListener | null = null;
    if (!this.httpRequestQueue)
    {
      // console.debug("creating DefaultHttpListener");
      const _defaultHttpListener = await DefaultHttpListener.create();
      defaultHttpListener = _defaultHttpListener;
      this.httpRequestQueue = createHttpRequestQueue();
      defaultHttpListener.setHttpRequestQueue(this.httpRequestQueue);
    }
    else
    {
      // console.debug("using given httpRequestQueue");
    }
    ...
    try
    {
      for (const auth of auths)
      {
        ...
        // check & respond on http requests
        const check = async () =>
          {
            while (checkLoop)
            {
              const rwr = await this.httpRequestQueue?.shift();
              if (!rwr)
              {
                return;
              }
              rwr.resolver(ACMEHttp.checkRequest(token, keyAuth, rwr.request));
            }
          };
        check();

        // tell acme server that the challenge is ready to be solved by our http request queue
        await this.session.post(challengeUrl,
          {
            payload: {},
            expectedAcmeStatus: [ ACMEStatus.processing, ACMEStatus.valid, ACMEStatus.pending ],
          });
    ...
    finally
    {
      ...
      {
        // console.debug("stopping defaultHttpListener");
        await defaultHttpListener.stop();
      }
